...
export function TemplatesManagment() {
  ...
  const handleDeleteTemplate = async (templateId) => {
    ...
    try {
      const response = await fetch(`${APP_URL}/form/deleteForm`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <div className="w-full  flex flex-col space-y-4 items-center justify-center my-4">
      ...
      <Card className="w-full sm:w-4/5 lg:w-3/5 h-auto ">
        {btnSelection ? (
          <>
            ...
            {templates?.length > 0 ? (
              <div className="gap-2 grid grid-cols-1 sm:grid-cols-3  mx-2 px-4 py-3 overflow-y-auto custom-scrollbar max-h-96  md:max-h-full ">
                {templates?.map((item, index) => (
                  <Card
                    ...
                    <CardBody className="overflow-visible p-0 space-y-2">
                      <div className="h-1/5 flex text-center items-center justify-center ">
                    ...
                    <CardFooter className="flex items-center justify-center w-full space-x-4"></CardFooter>
                  </Card>
                ))}
              </div>
            ) : (
              <div className="gap-2 mx-2 flex items-center justify-center px-4 py-3 overflow-y-auto custom-scrollbar h-full ">
                {authData?.userSettings?.language
                  ...
                  : "Aún no has creado una plantilla."}
              </div>
            )}
          </>
        ) : (
      ...
      <Modal
        ...
        <ModalContent>
          ...
          <ModalBody>{selectedTemplate?.title}</ModalBody>
          <ModalFooter>
            <Button
              color="primary"
              variant="light"
